...
import setup from "./HeadLecturerAssign.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/head/lecturer-assign");
export default function HeadLecturerAssign() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/head/lecturer-assign">
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"👩‍🏫 Danh sách giảng viên hướng dẫn"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-person-plus"></i>{" Danh sách GVHD"}</li>
      </ol>
    </nav>

    <div className="table-container">
      <div className="table-header d-flex gap-2"></div>
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"#"}</th>
              <th className="sortable" onClick={event => page.invoke("event4", event)}>{"Tên GVHD "}<i className="bi bi-arrow-down-up"></i></th>
              ...
              <th className="sortable" onClick={event => page.invoke("event8", event)}>{"Số SV "}<i className="bi bi-arrow-down-up"></i></th>
