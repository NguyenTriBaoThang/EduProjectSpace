...
import setup from "./HeadCourseList.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/head/course-list");
export default function HeadCourseList() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/head/course-list">
  ...
  <nav className="navbar navbar-expand-lg px-3">
    ...
    <div className="ms-auto d-flex align-items-center">
      <input type="text" className="search-box me-3" id="searchInput" placeholder={"🔍 Tìm học phần..."} onKeyUp={event => page.invoke("event1", event)} />
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"📚 Danh sách học phần"}</h2>
    </div>
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        <li className="breadcrumb-item"><a href="/head/dashboard"><i className="bi bi-house-door"></i></a></li>
        <li className="breadcrumb-item active"><i className="bi bi-book"></i>{" Danh sách học phần"}</li>
      </ol>
    </nav>
      ...
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"#"}</th>
              <th className="sortable" onClick={event => page.invoke("event3", event)}>{"Tên học phần "}<i className="bi bi-arrow-down-up"></i></th>
