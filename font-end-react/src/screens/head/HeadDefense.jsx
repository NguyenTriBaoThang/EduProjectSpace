...
import setup from "./HeadDefense.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/head/defense");
export default function HeadDefense() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/head/defense">
  ...
  <div className="profile-dropdown" id="profileDropdown">
    <div className="profile-header">
      <img src="/assets/static/medit/imgUser/avatar.jpg" alt="Head Avatar" />
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 id="schedule-title" className="fw-bold text-primary">Đang tải…</h2>
      <div>
        <button className="btn btn-success me-2" onClick={event => page.invoke("event3", event)}>{"Thêm lịch "}<i className="bi bi-plus-circle"></i></button>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-calendar"></i>{" Quản lý lịch bảo vệ"}</li>
      </ol>
    </nav>

    <div className="table-container">
      <div className="table-header d-flex gap-2">
        <input type="text" id="searchInput" className="form-control" placeholder={"🔍 Tìm nhóm..."} onKeyUp={event => page.invoke("event5", event)} />
      </div>
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"#"}</th>
              <th className="sortable" onClick={event => page.invoke("event6", event)}>{"Mã đề tài "}<i className="bi bi-arrow-down-up"></i></th>
              ...
              <th className="sortable" onClick={event => page.invoke("event9", event)}>{"Ngày bảo vệ "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event10", event)}>{"Địa điểm "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event11", event)}>{"Hội đồng "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
