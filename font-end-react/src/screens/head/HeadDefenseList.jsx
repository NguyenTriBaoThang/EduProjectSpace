...
import setup from "./HeadDefenseList.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/head/defense-list");
export default function HeadDefenseList() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/head/defense-list">
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"📋 Danh sách học phần cần quản lý lịch bảo vệ"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-list-ul"></i>{" Danh sách môn cần xếp lịch bảo vệ"}</li>
      </ol>
    </nav>
        ...
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              ...
              <th className="sortable" onClick={event => page.invoke("event10", event)}>{"Số nhóm đã xếp lịch "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
