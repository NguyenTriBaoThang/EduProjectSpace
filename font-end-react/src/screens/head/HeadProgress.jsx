...
import setup from "./HeadProgress.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/head/progress");
export default function HeadProgress() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/head/progress">
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"⏳ Theo dõi tiến độ"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-folder"></i>{" Chi tiết tiến độ"}</li>
      </ol>
    </nav>
        ...
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              ...
              <th className="sortable" onClick={event => page.invoke("event7", event)}>{"Mã đồ án "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event8", event)}>{"Tên đồ án "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event9", event)}>{"Thành viên "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event10", event)}>{"GVHD "}<i className="bi bi-arrow-down-up"></i></th>
