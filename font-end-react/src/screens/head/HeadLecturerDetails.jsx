...
import setup from "./HeadLecturerDetails.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/head/lecturer-details");
export default function HeadLecturerDetails() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/head/lecturer-details">
  ...
  <div className="sidebar-section">
    ...
    <a href="/font-end/head/head_briefing_courses.html"><i className="bi bi-award"></i>{" Duyệt chấm điểm"}</a>
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"👩‍🏫 Chi tiết giảng viên hướng dẫn"}</h2>
      <button className="btn btn-info" onClick={event => page.invoke("event3", event)}>{"Xuất Excel nhóm "}<i className="bi bi-file-earmark-excel"></i></button>
    </div>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-folder"></i>{" Chi tiết GVHD"}</li>
      </ol>
    </nav>

    <div className="card mb-4">
      <div className="card-header">
        <h5>{"Thông tin giảng viên\n                "}</h5></div>
      <div className="card-body">
        <p><strong>{"Tên GVHD:"}</strong>
          <span id="lecturerName"></span></p>
        <p><strong>{"Học phần:"}</strong>
          <span id="courseInfo"></span></p>
        <p><strong>{"Số sinh viên:"}</strong>
          <span id="studentCount"></span></p>
        <p><strong>{"Số nhóm:"}</strong>
    ...
    <div className="table-container">
      <div className="table-header d-flex gap-2">
        <input type="text" id="searchInput" className="form-control" placeholder={"🔍 Tìm kiếm nhóm..."} onInput={event => page.invoke("event4", event)} />
      </div>
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"ID Nhóm"}</th>
              <th className="sortable" onClick={event => page.invoke("event5", event)}>{"Mã SV "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event6", event)}>{"Tên SV "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event7", event)}>{"Tên nhóm "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event8", event)}>{"Tên đồ án "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
