...
import setup from "./HeadGradingCourses.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/head/grading-courses");
export default function HeadGradingCourses() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/head/grading-courses">
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"🏆 Danh sách học phần cần duyệt chấm điểm"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-award"></i>{" Duyệt chấm điểm"}</li>
      </ol>
    </nav>
        ...
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              ...
              <th className="sortable" onClick={event => page.invoke("event8", event)}>{"Mã lớp "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event9", event)}>{"Số nhóm "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event10", event)}>{"Số nhóm đã chấm "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event11", event)}>{"Số nhóm đã duyệt "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
