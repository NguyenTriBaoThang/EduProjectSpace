...
import setup from "./StudentGradesWeek.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/student/grades-week");
export default function StudentGradesWeek() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/student/grades-week">
  ...
  <div className="container mt-4">
    <h2 className="fw-bold text-primary" id="assignmentTitle">{"📖 Chi tiết điểm bài tập"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item">
          <a href="/student/grades-detail"><i className="bi bi-file-earmark-text"></i>{" Chi tiết điểm số"}</a>
        </li>
        <li className="breadcrumb-item active">
          <i className="bi bi-file-earmark-check"></i>{" Chi tiết bài tập\n                    "}</li>
      </ol>
    </nav>
    ...
    <div className="row mt-4">
      <div className="col-md-4">
        <div className="summary-card avg">
          <h4>{"📊 Điểm bài tập"}</h4>
          <h2 className="fw-bold" id="assignmentScore">Đang tải…</h2>
          <p>{"Đánh giá: "}<span className="fw-bold" id="assignmentEvaluation"></span></p>
          <p>{"Trạng thái: "}<span className="fw-bold" id="assignmentStatus"></span></p>
        </div>
      </div>
      <div className="col-md-8">
        <div id="submissionDetails" className="card p-4 shadow-sm">
          ...
          <div className="d-flex gap-2">
            ...
            <a href="/student/grades-detail" className="btn btn-outline-secondary"><i className="bi bi-arrow-left-circle"></i>{" Quay lại"}</a>
          </div>
        </div>
      </div>
    </div>
