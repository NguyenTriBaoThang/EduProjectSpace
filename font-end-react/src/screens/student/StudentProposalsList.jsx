...
import setup from "./StudentProposalsList.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/student/proposals-list");
export default function StudentProposalsList() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/student/proposals-list">
  ...
  <div className="container mt-4">
    <div className="mt-4 mb-4 d-flex justify-content-between align-items-center">
      <a href="/student/proposals-create" className="btn btn-success">{"Chuyển đến Đề xuất Đề tài"}</a>
    ...
    <h2 className="fw-bold text-primary">{"📋 Danh sách đề xuất đề tài đồ án"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active">
          <i className="bi bi-list-ul"></i>{" Danh sách đề xuất\n                    "}</li>
      </ol>
    </nav>
    <div className="table-container">

      <div className="table-header d-flex gap-2">
        <input type="text" id="searchInput" className="form-control" placeholder={"🔍 Tìm tên đề tài..."} onKeyUp={event => page.invoke("event1", event)} />
        <select id="statusFilter" className="form-select" onChange={event => page.invoke("event2", event)}>
          <option value="">{"📂 Trạng thái"}</option>
          <option value={"Chưa duyệt"}>{"Chưa duyệt"}</option>
          <option value={"Đã duyệt"}>{"Đã duyệt"}</option>
        </select>
        <select id="typeFilter" className="form-select" onChange={event => page.invoke("event3", event)}>
      ...
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"#"}</th>
              <th className="sortable" onClick={event => page.invoke("event4", event)}>{"Tên đề tài "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event5", event)}>{"Loại đồ án "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event6", event)}>{"Trạng thái "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
