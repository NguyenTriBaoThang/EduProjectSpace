...
import setup from "./AdminStudents.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/admin/students");
export default function AdminStudents() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/admin/students">
  ...
  <div className="dropdown-menu-wrapper">
    ...
    <div className="dropdown-content" style={{
      ...
      <a href="/admin/students" className="active"><i className="bi bi-people"></i>{" Sinh viên"}</a>
    </div>
  </div>
  ...
  <nav className="navbar navbar-expand-lg px-3">
    ...
    <div className="ms-auto d-flex align-items-center">
      <input type="text" className="search-box me-3" placeholder={"🔍 Tìm kiếm..."} id="searchInputFuse" onKeyUp={event => page.invoke("event2", event)} />
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"🎓 Quản lý sinh viên"}</h2>
      <div>
        <button className="btn btn-success me-2" data-bs-toggle="modal" data-bs-target="#addStudentModal">{"Thêm sinh viên "}<i className="bi bi-plus-circle"></i></button>
    ...
    <div className="table-container">
      <div className="table-header d-flex gap-2">
        <input type="text" id="searchInputFuse" className="form-control" placeholder={"🔍 Tìm sinh viên..."} onKeyUp={event => page.invoke("event7", event)} />
      ...
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"#"}</th>
              <th className="sortable" onClick={event => page.invoke("event9", event)}>{"Mã SV "}<i className="bi bi-arrow-down-up"></i></th>
              ...
              <th className="sortable" onClick={event => page.invoke("event12", event)}>{"Lớp "}<i className="bi bi-arrow-down-up"></i></th>
  ...
  <div className="modal-dialog">
    <div className="modal-content">
      <div className="modal-header">
        <h5 className="modal-title" id="addStudentModalLabel">{"Thêm sinh viên mới"}</h5>
      ...
      <div className="modal-body">
        <form id="addStudentForm">
          <div className="mb-3">
            ...
            <input type="text" id="studentId" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Họ và tên"}</label>
            <input type="text" id="studentName" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Email"}</label>
            <input type="email" id="studentEmail" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Khoa"}</label>
            <select id="studentDepartmentId" className="form-select" required={true}>
          ...
          <div className="mb-3">
            ...
            <input type="text" id="studentClass" className="form-control" required={true} />
          </div>
        </form>
      </div>
    ...
    <div className="modal fade" id="editStudentModal" tabIndex="-1" aria-labelledby="editStudentModalLabel" aria-hidden="true">
  <div className="modal-dialog">
    <div className="modal-content">
      <div className="modal-header">
        <h5 className="modal-title" id="editStudentModalLabel">{"Sửa thông tin sinh viên"}</h5>
        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div className="modal-body">
        <form id="editStudentForm">
          ...
          <div className="mb-3">
            <label className="form-label">{"Mã sinh viên"}</label>
            <input type="text" id="editStudentCode" className="form-control" readOnly={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Họ và tên"}</label>
            <input type="text" id="editStudentName" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Email"}</label>
            <input type="email" id="editStudentEmail" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Khoa"}</label>
            <select id="editStudentDepartmentId" className="form-select" required={true}>
              <option value="">{"Chọn khoa"}</option>
          ...
          <div className="mb-3">
            <label className="form-label">{"Lớp"}</label>
            <input type="text" id="editStudentClass" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Mật khẩu"}</label>
            <input type="password" id="editStudentPassword" className="form-control" />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Trạng thái"}</label>
            <select id="editStudentStatus" className="form-select" required={true}>
              <option value={"Hoạt động"}>{"Hoạt động"}</option>
              <option value={"Khóa"}>{"Khóa"}</option>
            </select>
          </div>
