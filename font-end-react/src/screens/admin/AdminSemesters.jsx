...
import setup from "./AdminSemesters.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/admin/semesters");
export default function AdminSemesters() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/admin/semesters">
  ...
  <a href="/admin/semesters" className="active"><i className="bi bi-calendar"></i>{" Quản lý kỳ học"}</a>
  ...
  <nav className="navbar navbar-expand-lg px-3">
    ...
    <div className="ms-auto d-flex align-items-center">
      <input type="text" className="search-box me-3" id="searchInputFuse" placeholder={"🔍 Tìm kỳ học..."} />
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"📅 Quản lý kỳ học"}</h2>
      <div>
        <button className="btn btn-success me-2" data-bs-toggle="modal" data-bs-target="#addSemesterModal">{"Thêm kỳ học "}<i className="bi bi-plus-circle"></i></button>
    ...
    <div className="table-container">
      <div className="table-header d-flex gap-2">
        <input type="text" id="searchInputFuse" className="form-control" placeholder={"🔍 Tìm kỳ học..."} onKeyUp={event => page.invoke("event4", event)} />
      ...
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"#"}</th>
              <th className="sortable" onClick={event => page.invoke("event6", event)}>{"Tên kỳ học "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event7", event)}>{"Ngày bắt đầu "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event8", event)}>{"Ngày kết thúc "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event9", event)}>{"Trạng thái "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
    ...
    <div className="modal fade" id="addSemesterModal" tabIndex="-1" aria-labelledby="addSemesterModalLabel" aria-hidden="true">
  <div className="modal-dialog">
    <div className="modal-content">
      <div className="modal-header">
        <h5 className="modal-title" id="addSemesterModalLabel">{"Thêm kỳ học mới"}</h5>
        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div className="modal-body">
        <form id="addSemesterForm">
          <div className="mb-3">
            <label className="form-label">{"Tên kỳ học"}</label>
            <input type="text" id="semesterName" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Ngày bắt đầu"}</label>
            <input type="date" id="semesterStartDate" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Ngày kết thúc"}</label>
            <input type="date" id="semesterEndDate" className="form-control" required={true} />
          </div>
        </form>
      </div>
      <div className="modal-footer">
        <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">{"Hủy"}</button>
        <button type="button" className="btn btn-primary" onClick={event => page.invoke("event10", event)}>{"Thêm"}</button>
      </div>
    </div>
  </div>
    </div>
    ...
    <div className="modal fade" id="editSemesterModal" tabIndex="-1" aria-labelledby="editSemesterModalLabel" aria-hidden="true">
  <div className="modal-dialog">
    <div className="modal-content">
      <div className="modal-header">
        <h5 className="modal-title" id="editSemesterModalLabel">{"Sửa thông tin kỳ học"}</h5>
        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div className="modal-body">
        <form id="editSemesterForm">
          ...
          <div className="mb-3">
            <label className="form-label">{"Tên kỳ học"}</label>
            <input type="text" id="editSemesterName" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Ngày bắt đầu"}</label>
            <input type="date" id="editSemesterStartDate" className="form-control" required={true} />
          </div>
          <div className="mb-3">
            <label className="form-label">{"Ngày kết thúc"}</label>
            <input type="date" id="editSemesterEndDate" className="form-control" required={true} />
          </div>
        </form>
      </div>
      <div className="modal-footer">
        <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">{"Hủy"}</button>
        <button type="button" className="btn btn-primary" onClick={event => page.invoke("event11", event)}>{"Lưu"}</button>
      </div>
    </div>
  </div>
    </div>
