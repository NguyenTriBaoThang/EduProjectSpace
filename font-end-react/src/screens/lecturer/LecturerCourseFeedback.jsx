...
import setup from "./LecturerCourseFeedback.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/lecturer/course-feedback");
export default function LecturerCourseFeedback() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/lecturer/course-feedback">
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"📚 Danh sách học phần cần phản hồi"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-chat-left-text"></i>{" Danh sách môn cần phản hồi"}</li>
      </ol>
    </nav>
      ...
      <div className="table-content">
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              <th>{"#"}</th>
              <th className="sortable" onClick={event => page.invoke("event3", event)}>{"Mã học phần "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event4", event)}>{"Tên học phần "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event5", event)}>{"Học kỳ "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event6", event)}>{"Mã khoa "}<i className="bi bi-arrow-down-up"></i></th>
              <th className="sortable" onClick={event => page.invoke("event7", event)}>{"Số đồ án "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
