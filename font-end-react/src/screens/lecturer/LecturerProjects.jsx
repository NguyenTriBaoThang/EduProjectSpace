...
import setup from "./LecturerProjects.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/lecturer/projects");
export default function LecturerProjects() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/lecturer/projects">
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"📂 Quản lý đồ án"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item"><a href="/lecturer/courses"><i className="bi bi-book"></i>{" Quản lý học phần"}</a></li>
        <li className="breadcrumb-item active"><i className="bi bi-folder"></i>{" Quản lý đồ án"}</li>
      </ol>
    </nav>
    <div className="table-container">
      <div className="table-header d-flex gap-2">
        <input type="text" id="searchInput" className="form-control" placeholder={"🔍 Tìm đồ án..."} onKeyUp={event => page.invoke("event3", event)} />
        ...
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              ...
              <th className="sortable" onClick={event => page.invoke("event8", event)}>{"Sinh viên "}<i className="bi bi-arrow-down-up"></i></th>
