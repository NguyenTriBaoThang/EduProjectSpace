...
import setup from "./LecturerCourseResources.controller";
import pages from "../../pageManifest.json";
const definition = pages.find(page => page.path === "/lecturer/course-resources");
export default function LecturerCourseResources() {
  const page = usePageController(setup, definition);
  return <div ref={page.root} className="page-frame" data-page="/lecturer/course-resources">
  ...
  <div className="container mt-4">
    <div className="d-flex justify-content-between align-items-center mb-4">
      <h2 className="fw-bold text-primary">{"📚 Danh sách học phần cần gợi ý tài liệu"}</h2>
    ...
    <nav className="breadcrumb-container">
      <ol className="breadcrumb">
        ...
        <li className="breadcrumb-item active"><i className="bi bi-book"></i>{" Danh sách môn cần gợi ý tài liệu"}</li>
      </ol>
    </nav>
        ...
        <table className="table-custom table-bordered table-hover">
          <thead>
            <tr>
              ...
              <th className="sortable" onClick={event => page.invoke("event8", event)}>{"Số tài liệu "}<i className="bi bi-arrow-down-up"></i></th>
              <th>{"Hành động"}</th>
            </tr>
          </thead>
