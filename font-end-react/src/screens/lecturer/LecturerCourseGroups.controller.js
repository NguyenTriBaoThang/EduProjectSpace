...
export default function setup(env) {
    ...
    if (paginatedData.length === 0) {
      ...
      paginatedData.forEach((course, index) => {
        tableBody.innerHTML += env.html(`
          ...
          viewGroups(String(course.courseId));
        })}">
  ...
  // Điều hướng đến trang chia nhóm
  // Điều hướng đến trang chia nhóm
  function viewGroups(courseId) {
    env.navigate(`lecturer_groups.html?courseId=${courseId}`);
  }
  ...
  function exportCourses() {
    const filteredCourses = getFilteredCourses();
    const worksheetData = [["Danh sách học phần cần chia nhóm - Hệ thống Sinh viên HUTECH"], [`Giảng viên hướng dẫn: ${fullNameLecturer}`], [], ["#", "Mã học phần", "Tên học phần", "Học kỳ", "Mã khoa", "Số SV", "Số nhóm"]];
    filteredCourses.forEach((course, index) => {
      worksheetData.push([index + 1, course.courseId, course.name, course.semester, course.facultyCode, course.studentCount, course.groupCount]);
    });
    const worksheet = XLSX.utils.aoa_to_sheet(worksheetData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "MonHocChiaNhom");
    XLSX.writeFile(workbook, "mon_hoc_can_chia_nhom.xlsx");
  }
  ...
  async function loadCourses() {
    try {
      const response = await fetch(`${API_URL}/api/LecturerCourseGroup/courses`, {
        method: "GET",
        headers: {
          "Accept": "*/*",
