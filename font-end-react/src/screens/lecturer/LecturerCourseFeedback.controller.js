...
export default function setup(env) {
  ...
  async function loadCourses() {
    try {
      const response = await fetch(`${API_URL}/api/LecturerFeedback`, {
        method: "GET",
        headers: {
          "Accept": "*/*",
      ...
      document.getElementById("tableBody").innerHTML = env.html("<tr><td colspan=\"7\" class=\"text-center\">Không tìm thấy học phần nào.</td></tr>");
    }
  }
    ...
    if (sortColumn) {
      filteredCourses = [...courses].sort((a, b) => {
        ...
        if (sortColumn === "projectCount") {
          valueA = a.projectCount;
          valueB = b.projectCount;
        } else {
    ...
    if (paginatedData.length === 0) {
      ...
      paginatedData.forEach((course, index) => {
        tableBody.innerHTML += env.html(`
          ...
          viewFeedback(String(course.courseId));
        })}">
  ...
  // Điều hướng đến trang phản hồi đồ án
  // Điều hướng đến trang phản hồi đồ án
  function viewFeedback(courseId) {
    env.navigate(`lecturer_feedback.html?courseId=${encodeURIComponent(courseId)}`);
  }

  // Xuất danh sách học phần sang Excel
  // Xuất danh sách học phần sang Excel
  function exportCourses() {
    const worksheetData = [["Danh sách học phần cần phản hồi - Hệ thống Sinh viên HUTECH"], [`Giảng viên hướng dẫn: ${fullNameLecturer}`], [], ["#", "Mã học phần", "Tên học phần", "Học kỳ", "Mã khoa", "Số đồ án"]];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MonHocPhanHoi");
    XLSX.writeFile(workbook, "mon_hoc_can_phan_hoi.xlsx");
  }
