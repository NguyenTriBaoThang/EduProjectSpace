...
namespace EduProject_TADProgrammer.Services
{
    public class DefenseCommitteeService
    {
        ...
        public DefenseCommitteeService(ApplicationDbContext context)
        {
            _context = context;
        }

        // Lấy danh sách hội đồng
        public async Task<List<DefenseCommitteeDto>> GetAllCommittees()
        {
            return await _context.DefenseCommittees
                .Include(dc => dc.Semester)
                .Include(dc => dc.CommitteeMembers)
                    .ThenInclude(cm => cm.Lecturer)
                .Select(dc => new DefenseCommitteeDto
                {
                    Id = dc.Id,
                    Name = dc.Name,
                    SemesterId = dc.SemesterId,
                    SemesterName = dc.Semester.Name,
                    Members = dc.CommitteeMembers.Select(cm => new CommitteeMemberDto
                    {
        ...
        public async Task<DefenseCommitteeDto> GetCommitteeById(long id)
        {
            ...
            return new DefenseCommitteeDto
            {
                Id = committee.Id,
                Name = committee.Name,
                SemesterId = committee.SemesterId,
                SemesterName = committee.Semester.Name,
                Members = committee.CommitteeMembers.Select(cm => new CommitteeMemberDto
                {
                    LecturerId = cm.LecturerId,
                    FullName = cm.Lecturer.FullName,
                    Role = cm.Role
        ...
        // Thêm hội đồng mới
        public async Task<DefenseCommitteeDto> CreateCommittee(CreateDefenseCommitteeRequest request)
        {
        ...
        // Cập nhật hội đồng
        public async System.Threading.Tasks.Task UpdateCommittee(long id, UpdateDefenseCommitteeRequest request)
        {
            var committee = await _context.DefenseCommittees
                .Include(dc => dc.CommitteeMembers)
                .FirstOrDefaultAsync(dc => dc.Id == id);
